...
def load_brisbane_suburbs():
    with open('postcode.csv') as csv_file:
        csv_reader = csv.reader(csv_file, delimiter=',')
        for row in csv_reader:
            suburbs.append(f'{row[1].title()}, QLD {row[0]}')
...
## ----------------End Display---------------------------------
## -------------------------------------------------------------
@app.route("{}/suburb".format(url_prex), methods=['GET'])
def get_suburb():
    """
    This method shows all the members of a lifegroup.
    """
    ...
    for suburb in suburbs:
        if checked & (name not in suburb.lower()):
            break
        if name in suburb.lower():
            list.append({"suburb": suburb})
            checked = True
